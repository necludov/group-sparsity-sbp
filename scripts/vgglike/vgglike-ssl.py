...
def main(_):
    ...
    if summaries_dir == '':
        summaries_dir = './logs/vgglike_ssl_{}'.format(FLAGS.dataset)
        summaries_dir += time.strftime('_%d-%m-%Y_%H:%M:%S')
    checkpoints_dir = FLAGS.checkpoints_dir
    if checkpoints_dir == '':
        checkpoints_dir = './checkpoints/vgglike_ssl_{}'.format(FLAGS.dataset)
    ...
    with tf.Graph().as_default() as graph, tf.device('/cpu:0'):
        ...
        not_conv_variables = filter(lambda v: not v.name.startswith('conv_'), variables)
        map(lambda v: final_variables.__setitem__(v.name[:-2], v), not_conv_variables)
        for conv_number in range(1,14):
            conv_vars = filter(lambda v: v.name.startswith('conv_%d/' % conv_number), variables)
            kernel = filter(lambda v: 'kernel' in v.name, conv_vars)[0]
            biases = filter(lambda v: 'biases' in v.name, conv_vars)[0]
            loss = tf.sqrt(tf.reduce_sum(tf.nn.bias_add(kernel, biases)**2, [0, 1, 2])+1e-16)
            mask = tf.cast(tf.greater(loss, 1e-5*tf.ones_like(loss)), tf.bool)
            reduced_size_ops.append(tf.reduce_sum(tf.cast(mask, tf.int32)))
            reduced_kernel = tf.transpose(tf.boolean_mask(tf.transpose(kernel, [3, 0, 1, 2]), mask), [1, 2, 3, 0])
            if conv_number > 1:
                reduced_kernel = tf.transpose(tf.boolean_mask(tf.transpose(reduced_kernel, [2, 0, 1, 3]),
                                                              previous_mask), [1, 2, 0, 3])
            assign_ops.append(tf.assign(kernel, reduced_kernel, validate_shape=False))
            final_variables[kernel.name[:-2]] = kernel
            flat_vars = filter(lambda v: 'kernel' not in v.name, conv_vars)
            for v in flat_vars:
                reduced_v = tf.boolean_mask(v, mask)
                assign_ops.append(tf.assign(v, reduced_v, validate_shape=False))
                final_variables[v.name[:-2]] = v
            previous_mask = mask
        final_saver = tf.train.Saver(final_variables)
    ...
    with tf.Session(config=config, graph=graph) as sess:
        ...
        if FLAGS.checkpoint != '':
            variables_to_restore = filter(lambda v: 'reduced' not in v.name, tf.get_collection('variables'))
        ...
        reduced_filter_sizes = sess.run(reduced_size_ops)
        print reduced_filter_sizes
        _ = sess.run(assign_ops)
        final_saver.save(sess, checkpoints_dir + '/final_model.ckpt')
        coord.request_stop()
        coord.join(threads)
