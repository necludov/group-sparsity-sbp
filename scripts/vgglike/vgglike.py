...
def net_vgglike(images, nclass, scale, is_training, reuse):
    net = conv_bn_rectify(images, int(64*scale), 'conv_1', is_training, reuse)
    net = tf.contrib.layers.dropout(net, keep_prob=0.7, is_training=is_training)
    net = conv_bn_rectify(net, int(64*scale), 'conv_2', is_training, reuse)
    net = tf.nn.max_pool(net, ksize=[1,2,2,1], strides=[1,2,2,1], padding='VALID')

    net = conv_bn_rectify(net, int(128*scale), 'conv_3', is_training, reuse)
    net = tf.contrib.layers.dropout(net, keep_prob=0.6, is_training=is_training)
    net = conv_bn_rectify(net, int(128*scale), 'conv_4', is_training, reuse)
    net = tf.nn.max_pool(net, ksize=[1,2,2,1], strides=[1,2,2,1], padding='VALID')
...
def main(_):
    ...
    if summaries_dir == '':
        summaries_dir = './logs/vgglike_{}_scale{}'.format(FLAGS.dataset, FLAGS.scale)
        summaries_dir += time.strftime('_%d-%m-%Y_%H:%M:%S')
    checkpoints_dir = FLAGS.checkpoints_dir
    if checkpoints_dir == '':
        checkpoints_dir = './checkpoints/vgglike_{}_scale{}'.format(FLAGS.dataset, FLAGS.scale)
    ...
    with tf.Graph().as_default() as graph, tf.device('/cpu:0'):
        ...
        loss = lambda preds, labels, reuse: metrics.log_loss(preds, labels, reuse, n_train_examples, wd)
        ...
        lr_policy = lambda epoch_num: policies.linear_decay(epoch_num, decay_start=100, total_epochs=n_epochs,
                                                            start_value=1e-3)
        wd_policy = lambda epoch_num: policies.linear_growth(epoch_num, growth_start=0, total_epochs=n_epochs,
                                                             start_value=1e-3, end_value=1.0)
